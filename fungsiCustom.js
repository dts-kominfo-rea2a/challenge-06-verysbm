...
const bacaData = (fnCallBack) => {
  ...
  fs.readFile(file1, { encoding: "utf8" }, (err, data) => {
    ...
    resultArr.push(dataResult.message.split(" ")[1]);
  });
  ...
  fs.readFile(file2, { encoding: "utf8" }, (err, data) => {
    ...
    const dataResult = JSON.parse(data);
    resultArr.push(dataResult[0].message.split(" ")[1]);
  });
  ...
  fs.readFile(file3, { encoding: "utf8" }, (err, data) => {
    ...
    const dataResult = JSON.parse(data);
    resultArr.push(dataResult[0].data.message.split(" ")[1]);
    fnCallBack(null, resultArr);
  });
};
